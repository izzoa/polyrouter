...
- **spec.md §12** — regenerated from the config registry: every registered env var grouped by namespace with its default, the four required-in-production hex secrets and the localhost-default `DATABASE_URL`/`REDIS_URL` marked, the loopback dev-secret fallbacks noted, the stale cloud-only vars dropped, and the wrong `ROUTING_AUTO_LAYERS` default corrected.
