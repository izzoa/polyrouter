...
7. **Auto routing — Layer 3 (cascade)** — cheap-first + escalation. (Self-host feature target reached: L0 + L1 + L3.)
   → **Workload axis (Epic W, shipped 2026-08-22; spec `workload-routing` + deltas in `structural-routing` / `semantic-routing` / `cascade-routing` / `routing-config` / `dashboard-config` / `analytics-api` / `dashboard-analytics` / `semantic-learning`):** `auto` additionally records a **workload** class — `code` / `vision` / `structured` (structural, baseline) and, via the flag-gated semantic module only, `research` / `writing` — plus the telemetry-only `none`. W-1 `add-workload-telemetry` records it; W-2 `add-workload-routing` routes it (`auto_workload` rules claim a class ahead of the bands, `decision_layer='workload'`); W-3 `add-semantic-workloads` adds the reserved classes (one embed shared with L2, margin/min-sim rails); W-4 `add-workload-scoped-bands` lets `auto_high`/`auto_low` carry a class scope (the scoped rule decides — no silent fall-back; cascade within a class; the recorded reason ends with ` scope=<class>`). It is a second verdict from the L1/L2 mechanisms — **not an L4**, no new header, no new `ROUTING_AUTO_LAYERS` token — the taxonomy is fixed and bumps only by a change, and detection is **never keyword-based** (invariant 9).
...
4. **Cost is immutable.** Compute cost at request time against **snapshotted unit prices** stored on the RequestLog; never recompute historical cost against current prices. Flag missing/estimated usage (`usage_estimated`), never store silent nulls. Recorded prices come from a bundled versioned table; a provider's captured **`listed`** price (from `sync-models`) MAY serve as a clearly-marked estimate **fallback** ONLY when the catalog (exact + native-family) is unknown — never overriding it — and is snapshotted immutably like any source (record-listed-price-fallback). (§7.7)
