...
- **Scope:** Better Auth email/password + Google/GitHub/Discord OAuth; session guard for `/api`; **first user = admin with transaction/advisory-lock race guard**; `MODE=selfhosted` localhost auto-login + `SEED_DATA`; rate limiting on register/login/password-reset; Agent CRUD (create mints `poly_…` key shown once, rotate, delete, `last_used_at`), stored as **HMAC-SHA256 hash + prefix for O(1) lookup — never bcrypt**; Bearer-key proxy guard resolving prefix → HMAC compare → Agent; per-harness connection snippet data from `shared`.
- **Out of scope:** the proxy endpoints themselves (#10); dashboard UI (#18); org/team membership (deferred).
...
- **DoD (§15):** API keys fast-verified (HMAC + prefix, benchmarked no slow hash on the hot path); concurrent first-signup race yields exactly one admin; auth endpoints throttle; session passwords use argon2/bcrypt; cross-tenant tests cover Agents.
