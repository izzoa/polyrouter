...
const QUEUE_NAME = 'semantic-learning';
const SCHEDULER_ID = 'semantic-learning';
const JOB_NAME = 'semantic_learning_sweep';
const RECONCILE_TIMEOUT_MS = 3_000;
const RECONCILE_RETRY_MS = 30_000;

/**
 * Semantic-learning sweep scheduler (add-semantic-learning task 4.2), on its OWN
 * dedicated BullMQ queue — mirrors the calibration scheduler's discipline: a
 * fail-open bootstrap (a down Redis never gates boot or affects routing —
 * invariant 1), an always-created producer Queue (a disabled node can still
 * remove a stale schedule), a consuming Worker ONLY when
 * `SEMANTIC_LEARNING_SCHED_ENABLED`, and a bounded shutdown. When Layer 2 is off
 * (no model, classifier unavailable) the occurrence no-ops cleanly.
 */
@Injectable()
export class SemanticLearningScheduler implements OnApplicationBootstrap, OnApplicationShutdown {
  private readonly logger = new Logger('SemanticLearningScheduler');
  private readonly producerConn: Redis;
  private readonly workerConn: Redis | undefined;
  private readonly storeConn: Redis;
  ...
  constructor(
    ...
    this.enabled = cfg.learning.schedEnabled;
    const { auth, base } = loadAuthConfig();
    this.tenantKey = deriveTenantHmacKey(resolveAuthSecrets(auth, base).apiKeyHmacSecret);
    // A dedicated fail-fast connection for the store's ops (never hang the sweep).
    this.storeConn = redis.duplicate({ enableOfflineQueue: false, maxRetriesPerRequest: 1 });
    this.storeConn.on('error', () => {});
    if (this.storeConn.status === 'wait') void this.storeConn.connect().catch(() => {});
    this.store = new RedisLearningStore(this.storeConn);
    ...
    if (this.enabled) {
      ...
      this.worker.on('failed', (job, err) =>
        this.logger.warn(
          `semantic learning sweep ${job?.id ?? '?'} failed: ${String(err?.message ?? 'error')}`,
        ),
      );
    }
  }
  ...
  private async reconcile(): Promise<void> {
    ...
    try {
      ...
      this.logger.warn(
        `semantic learning scheduler reconcile deferred: ${String((err as Error).message)}`,
      );
  ...
  private async applySchedule(): Promise<void> {
    if (this.enabled) {
      await this.queue.upsertJobScheduler(
        SCHEDULER_ID,
        { pattern: this.cfg.learning.schedCron, tz: 'UTC' },
        {
          name: JOB_NAME,
  ...
  runOccurrence(now: number): Promise<unknown> {
    return runSemanticLearningOccurrence(
      this.db,
      this.store,
      this.classifier.learningProvenance,
      (principal) => loadRoutingSnapshot(this.db, principal).then((r) => r.snapshot),
      this.cfg.learning,
      this.routing.cascade.qualityThreshold,
      (ownerUserId) => tenantHmac(this.tenantKey, ownerUserId),
      now,
      this.logger,
    );
  }
